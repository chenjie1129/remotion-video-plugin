 ...
 */
import { defineRemotionTool } from './host-contracts.js';
import { browserArguments, inspectArtifact, inspectProject, normalizeToolsConfig, parseCompositionIds, prepareOutput, probeMedia, propsArgument, requireCompositionId, resolveEntryPoint, runManaged, } from './remotion-runtime.js';
...
export function apply(baseCtx, rawConfig = {}) {
    ...
    ctx.tools.register(defineRemotionTool({
        ...
        async execute(args, exec) {
            ...
            return {
                status: checks.some(check => check.status === 'fail') ? 'needs-setup' : 'ready',
    ...
    ctx.tools.register(defineRemotionTool({
        ...
        async execute(args, exec) {
            const project = await inspectProject(exec, optionalString(args, 'project_path'));
            if (project.remotionBin === undefined)
                throw new Error('local Remotion CLI is missing; run remotion_doctor');
    ...
    ctx.tools.register(defineRemotionTool({
        ...
        async execute(args, exec) {
            ...
            if (!Number.isInteger(frame) || frame < 0 || frame > 10_000_000)
                throw new Error('frame must be an integer between 0 and 10000000');
    ...
    ctx.tools.register(defineRemotionTool({
        ...
        async execute(args, exec) {
            ...
            if (!['h264', 'vp9', 'prores', 'gif'].includes(requestedCodec))
                throw new Error('codec must be h264, vp9, prores, or gif');
            const codec = requestedCodec;
            const extensions = { h264: ['.mp4'], vp9: ['.webm'], prores: ['.mov'], gif: ['.gif'] };
