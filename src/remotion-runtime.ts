...
import type { HarnessContext, SubprocessSpawnSpec, ToolExecutionContext } from './host-contracts.js'
...
export function normalizeToolsConfig(config: ToolsConfig = {}): NormalizedToolsConfig {
...
async function existingDirectoryWithin(root: string, input: string, label: string): Promise<string> {
  if (input.includes('\0') || isAbsolute(input)) throw new Error(`${label} must be a relative path inside the session workspace`)
  const target = await realpath(resolve(root, input))
  if (!within(root, target)) throw new Error(`${label} resolves outside the session workspace`)
  if (!(await stat(target)).isDirectory()) throw new Error(`${label} is not a directory`)
  return target
}

async function existingFileWithin(root: string, input: string, label: string): Promise<string> {
  if (input.includes('\0') || isAbsolute(input)) throw new Error(`${label} must be a relative path`)
  const target = await realpath(resolve(root, input))
  if (!within(root, target)) throw new Error(`${label} resolves outside its allowed directory`)
  if (!(await stat(target)).isFile()) throw new Error(`${label} is not a file`)
  return target
}
...
async function readPackage(path: string): Promise<PackageJson | undefined> {
  try {
    const parsed = JSON.parse(await readFile(path, 'utf8')) as unknown
    return typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed) ? parsed as PackageJson : undefined
  } catch {
    return undefined
  }
}

function dependencyVersion(packageJson: PackageJson | undefined, name: string): string | undefined {
  const value = packageJson?.dependencies?.[name] ?? packageJson?.devDependencies?.[name]
  return typeof value === 'string' ? value : undefined
}

async function detectPackageManager(project: string): Promise<ProjectContext['packageManager']> {
  const candidates: Array<[ProjectContext['packageManager'], string]> = [
    ['pnpm', 'pnpm-lock.yaml'],
    ['yarn', 'yarn.lock'],
    ['bun', 'bun.lock'],
    ['bun', 'bun.lockb'],
  ...
  for (const [manager, lock] of candidates) {
    if (await optionalFile(join(project, lock))) return manager
  }
  return 'unknown'
}

async function localExecutable(project: string, name: string): Promise<string | undefined> {
  const candidate = join(project, 'node_modules', '.bin', process.platform === 'win32' ? `${name}.cmd` : name)
  try {
    const target = await realpath(candidate)
    if (!within(project, target) || !(await stat(target)).isFile()) return undefined
    return target
  } catch {
    return undefined
  }
...
async function bundledFfprobe(project: string): Promise<string | undefined> {
  ...
  for (const name of names.filter(value => value.startsWith('compositor-')).sort()) {
    ...
    try {
      const target = await realpath(candidate)
      if (within(project, target) && (await stat(target)).isFile()) return target
    } catch {
...
export async function inspectProject(exec: ToolExecutionContext, projectPath = '.'): Promise<ProjectContext> {
  const requestedWorkspace = exec.agent?.session.header.cwd ?? process.cwd()
  const workspace = await realpath(requestedWorkspace)
  if (!(await stat(workspace)).isDirectory()) throw new Error('session workspace is not a directory')
...
export async function resolveEntryPoint(project: ProjectContext, requested?: string): Promise<{ absolute: string; relative: string }> {
...
export function requireCompositionId(value: string): string {
...
export function propsArgument(value: string | undefined, maxBytes: number): string[] {
  if (value === undefined) return []
  if (Buffer.byteLength(value, 'utf8') > maxBytes) throw new Error(`props_json exceeds the ${maxBytes}-byte limit`)
  let parsed: unknown
  try {
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) throw new Error('props_json must encode a JSON object')
...
export async function prepareOutput(
  ...
  if (!within(project.workspace, absolute)) throw new Error('output_path escapes the session workspace')
  ...
  if (!within(project.workspace, existingAncestor)) throw new Error('output_path follows a directory symlink outside the session workspace')
  await mkdir(dirname(absolute), { recursive: true })
  const outputDirectory = await realpath(dirname(absolute))
  if (!within(project.workspace, outputDirectory)) throw new Error('output_path resolves outside the session workspace')
  try {
    const current = await lstat(absolute)
    if (current.isSymbolicLink()) throw new Error('output_path must not be a symbolic link')
    if (!current.isFile()) throw new Error('output_path exists and is not a regular file')
    if (!overwrite) throw new Error('output_path already exists; set overwrite=true only when replacement is intended')
  } catch (error: unknown) {
    if (!(error instanceof Error) || !('code' in error) || error.code !== 'ENOENT') throw error
  }
...
export async function browserArguments(
  ctx: HarnessContext,
  exec: ToolExecutionContext,
  config: NormalizedToolsConfig,
): Promise<string[]> {
  if (config.browserExecutable === undefined) return []
...
export async function runManaged(
  ...
  if (stdout === undefined || stderr === undefined) throw new Error('managed Remotion command did not expose collected output')
  exec.signal.throwIfAborted()
...
export function parseCompositionIds(output: string): string[] {
  const lines = output.split(/\r?\n/).map(line => line.trim()).filter(Boolean)
  const candidate = lines.at(-1) ?? ''
  const ids = candidate.split(/\s+/).filter(value => COMPOSITION_ID.test(value))
  if (ids.length === 0) throw new Error('Remotion returned no parseable composition IDs')
  return [...new Set(ids)]
}
...
export async function inspectArtifact(project: ProjectContext, path: string, kind: Artifact['kind']): Promise<Artifact> {
  const target = await realpath(path)
  if (!within(project.workspace, target)) throw new Error('rendered artifact resolves outside the session workspace')
  const info = await stat(target)
  if (!info.isFile() || info.size <= 0) throw new Error('rendered artifact is missing or empty')
...
export async function probeMedia(
  ...
  if (project.ffprobeBin === undefined) throw new Error('the project does not contain Remotion\'s bundled ffprobe binary')
  ...
  try {
    document = JSON.parse(run.stdout) as FfprobeDocument
  } catch {
